...
namespace ACS.Core.Infrastructure
{
    ...
    public class EngineContext
    {
        ...
        protected static IEngine CreateEngineInstance(NopConfig config)
        {
            if (config != null && !string.IsNullOrEmpty(config.EngineType))
            {
                ...
                if (!typeof(IEngine).IsAssignableFrom(engineType))
                    throw new ConfigurationErrorsException("The type '" + engineType + "' doesn't implement 'ACS.Core.Infrastructure.IEngine' and cannot be configured in /configuration/nop/engine[@engineType] for that purpose.");
        ...
        public static IEngine Initialize(bool forceRecreate)
        {
            ...
            {
                var config = ConfigurationManager.GetSection("ACSConfig") as NopConfig;
