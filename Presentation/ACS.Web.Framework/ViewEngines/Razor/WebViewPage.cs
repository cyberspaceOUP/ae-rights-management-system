...
using ACS.Web.Framework.Localization;
using ACS.Services.Configuration;
using System.Linq;
//using ACS.Web.Framework.Themes;

namespace ACS.Web.Framework.ViewEngines.Razor
{
    public abstract class WebViewPage<TModel> : System.Web.Mvc.WebViewPage<TModel>
    {
       ...
       // private IWorkContext _workContext;
        ...
        public Localizer S
        {
            get
            {
                if (_localizer != null)
                {
                    ...
                    _localizer = (format, args) =>
                    {
                        var _setting = EngineContext.Current.Resolve<ISettingService>();
                        //var resFormat = _localizationService.GetResource(format);
                        var resFormat = _setting.GetAllSettings().Where(i => i.Name == format).FirstOrDefault().Value;
                        if (string.IsNullOrEmpty(resFormat))
                        {
                            return new LocalizedString(format);
                        }
        ...
        public int GetSelectedTabIndex()
        {
            ...
            int index = 0;
            string dataKey = "ACS.selected-tab-index";
